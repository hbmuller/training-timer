...
const inputTime = document.querySelector<HTMLInputElement>("#input-seconds")!;
const inputReps = document.querySelector<HTMLInputElement>("#input-times")!;
const buttonStart = document.querySelector<HTMLButtonElement>("#button-start")!;
...
let endTime = 0;
let nextLapTime = 0;
let stopInterval: null | (() => void) = null;
...
const leadingZeros = (value: number, length = 2) => {
...
const updateTimer = () => {
  ...
  if (totalTimeLeft <= 0) return stopTimer();
  if (currentTime >= nextLapTime) {
    beepLap();
    nextLapTime = startTime + (lapsCount + 1) * duration;
  }

  fieldLapTimer.innerHTML = `${formatDuration(totalLapLeft)} (${
